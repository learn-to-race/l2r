...
if __name__ == '__main__':
    ...
    for epoch in range(num_epochs):
        ...
        print(
            f'#{epoch + 1} train_loss: {train_loss:.6f}, test_loss: {test_loss:.6f}')
        ...
        orig_img = torch.as_tensor(
            imgs[test_indices[0]] / 255., device=device, dtype=torch.float)
        vae_img = vae(orig_img[None])[0][0]
        # (C, H, W)/RGB -> (H, W, C)/BGR
        cv2.imwrite(
            "orig.png",
            orig_img.detach().cpu().numpy()[
                ::-
                1].transpose(
                1,
                2,
